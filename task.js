...
function createMinerAccount( userAddress ){
    const address = userAddress
    const balance = 100
    return { address, balance }
}

const miner = createMinerAccount('0x1234')
...
function mine(miner, chainNetwork) {
    ...
    setTimeout(() => {
        chainNetwork.miningPool.forEach(poolData => {
            const { newBlock, sender } = poolData;
            newBlock.previousHash = getTheLatestBlock(chainNetwork).blockHash;
            chainNetwork.blockchain.push(newBlock);
...
mine(miner, bracuChainNetwork); // Start mining the pending blocks
